...
<!--Query Result-->
    <div class = 'treatmentOptions'>
        <div class = 'option1'>
           <div class = optionTitle> Tests</div>
               <div class = optionBody>
                <center>
                <?php
                    ...
                    $sql = "SELECT Date_given, Doctor_name, Test_type, Result
                            FROM MEDICAL_TEST
                            WHERE MEDICAL_TEST.Patient_SSN =  '".$ssn."';";

                    $result = mysqli_query($conn,$sql);
                    print "<pre>";
                    print "<table border=1>";
                    print "<tr><td>Date Administered </td><td> Doctor </td><td>Test Type </td><td> Result </td>";
                    while($row = mysqli_fetch_array($result, MYSQLI_BOTH))
                    {
                        print "\n";
                        print "<tr><td>$row[Date_given] </td><td>$row[Doctor_name] </td><td>$row[Test_type] </td><td> $row[Result]  </td></tr>    ";
                    }
                    print "</table>";
                    print "</pre>";
